<#
.SYNOPSIS
    Parses Exchange IMAP logs to extract unique client IPs with progress bar.
...
Write-Host "Parsing Exchange IMAP logs from folder: $LogFolder"
...
$outputFile = Join-Path -Path (Get-Location) -ChildPath "ParsedIPs-IMAP.txt"
...
foreach ($file in $files) {
    $count++
    Write-Progress -Activity "Scanning IMAP logs" -Status "$count of $total" -PercentComplete (($count / $total) * 100)
